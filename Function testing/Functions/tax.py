def calc_tax(amount, tax_rate, age):
    ...
    if not isinstance(amount, (int, float)):
        raise TypeError('The amount value must be int or float.')
    if not amount >= 0:
        raise ValueError('The amount mus be positive.')

    if not isinstance(tax_rate, float):
        raise TypeError('The tax_rate value must be int or float.')
    if not 0 < tax_rate < 1:
        raise ValueError('The tax_rate mus be positive.')
